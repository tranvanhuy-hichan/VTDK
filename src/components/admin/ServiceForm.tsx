        ...
        <div className="flex items-center gap-2.5">
          {isEditing && (
            <button
              ...
              className="bg-red-50 dark:bg-red-950/60 hover:bg-red-100 dark:hover:bg-red-900/60 text-red-600 dark:text-red-400 border border-red-200 dark:border-red-800/80 font-extrabold px-3.5 py-2 rounded-xl text-xs sm:text-sm transition-colors flex items-center gap-1.5 disabled:opacity-50 !min-h-0 cursor-pointer"
            >
              <Trash2 className="w-4 h-4" />
              <span className="hidden sm:inline">Xóa giải pháp</span>
            </button>
          )}

          <Link
            href="/admin/services"
            className="bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 font-extrabold px-4 py-2 rounded-xl text-xs sm:text-sm transition-colors !min-h-0 text-center"
          >
            Hủy bỏ
          </Link>
          <button
            ...
            disabled={isSubmitting}
            className="bg-[#075FA8] hover:bg-[#0B1F33] text-white font-extrabold px-5 py-2 rounded-xl text-xs sm:text-sm shadow-md transition-all flex items-center justify-center gap-2 disabled:opacity-50 !min-h-0 cursor-pointer"
          >
            {isSubmitting ? (
              <>
              ...
              <span>{isEditing ? "CẬP NHẬT GIẢI PHÁP" : "LƯU GIẢI PHÁP"}</span>
            )}
          </button>
        </div>
      </div>
      ...
      <form id="service-form" onSubmit={handleSubmit} className="space-y-6">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
          ...
          <div className="lg:col-span-7 space-y-6">
            ...
            <div className="bg-white dark:bg-slate-900 p-5 sm:p-6 rounded-2xl border border-slate-200/90 dark:border-slate-800 shadow-2xs space-y-4 transition-colors">
              <div className="flex items-center gap-2 border-b border-slate-100 dark:border-slate-800 pb-3">
                <span className="w-6 h-6 rounded-full bg-blue-100 dark:bg-blue-950 text-[#075FA8] dark:text-blue-300 font-black text-xs flex items-center justify-center">
                  1
                </span>
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full text-sm bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-2.5 text-slate-900 dark:text-white font-bold focus:bg-white dark:focus:bg-slate-900 focus:outline-none focus:border-[#075FA8] focus:ring-1 focus:ring-[#075FA8] transition-all"
                />
              </div>
                  ...
                  {SERVICE_ICONS.map((item) => {
                    ...
                    return (
                      <button
                        ...
                        className={`p-3 rounded-xl border flex flex-col items-center justify-center text-center gap-2 transition-all cursor-pointer !min-h-0 ${
                          isSelected
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full text-sm bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl p-3.5 text-slate-900 dark:text-white focus:bg-white dark:focus:bg-slate-900 focus:outline-none focus:border-[#075FA8] focus:ring-1 focus:ring-[#075FA8] transition-all leading-relaxed font-medium"
                />
              </div>
            </div>
            ...
            <div className="bg-white dark:bg-slate-900 p-5 sm:p-6 rounded-2xl border border-slate-200/90 dark:border-slate-800 shadow-2xs space-y-4 transition-colors">
              <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-3">
                ...
                <button
                  ...
                  className="inline-flex items-center gap-1 text-xs font-extrabold text-[#075FA8] dark:text-blue-400 hover:text-[#0B1F33] bg-blue-50 dark:bg-blue-950/60 border border-blue-200 dark:border-blue-800/80 hover:bg-blue-100 dark:hover:bg-blue-900/60 px-3 py-1.5 rounded-lg transition-colors cursor-pointer !min-h-0"
                >
              ...
              <div className="space-y-2.5">
                {features.map((feat, index) => (
                  <div key={index} className="flex items-center gap-2.5">
                    <span className="w-6 h-6 rounded-full bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 font-bold text-xs flex items-center justify-center shrink-0">
                      #{index + 1}
                    </span>
                    <input
                      type="text"
                      ...
                      className="flex-1 text-sm bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl px-3.5 py-2 text-slate-900 dark:text-white font-bold focus:bg-white dark:focus:bg-slate-900 focus:outline-none focus:border-[#075FA8]"
                    />
                    <button
                      type="button"
                      onClick={() => handleRemoveFeature(index)}
                      className="p-2 text-slate-400 hover:text-red-600 dark:hover:text-red-400 hover:bg-red-50 dark:hover:bg-red-950/60 rounded-xl transition-colors !min-h-0 cursor-pointer"
