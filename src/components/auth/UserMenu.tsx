...
import { User, LogIn, ShoppingBag, ShieldCheck, LogOut, ChevronDown, Moon, Sun, X, Settings } from "lucide-react";
...
export const UserMenu: React.FC = () => {
  ...
  return (
    <div className="relative shrink-0" ref={dropdownRef}>
      ...
      {isOpen && (
        <div className="absolute right-0 mt-2 w-64 max-w-[90vw] bg-white dark:bg-slate-900 rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-800 p-2 z-50 animate-in fade-in zoom-in-95 text-left">
          <div className="px-3 py-2 border-b border-slate-100 dark:border-slate-800">
            <p className="text-xs font-bold text-slate-900 dark:text-white truncate">{user.name}</p>
            <p className="text-[11px] text-slate-500 dark:text-slate-400 truncate">{user.email}</p>
            {user.role === "ADMIN" && (
              <span className="inline-block mt-1 px-2 py-0.5 bg-amber-100 dark:bg-amber-950/60 text-amber-800 dark:text-amber-300 text-[10px] font-extrabold rounded-md">
                Quản trị viên
              </span>
            )}
          </div>
          ...
          <div className="py-1 space-y-0.5">
            ...
            {user.role === "ADMIN" && (
              <Link
                href="/admin"
                onClick={() => setIsOpen(false)}
                className="flex items-center gap-2.5 px-3 py-2 rounded-xl text-xs font-bold text-amber-700 dark:text-amber-400 hover:bg-amber-50 dark:hover:bg-amber-950/40 transition-colors"
              >
