...
export const Services: React.FC<ServicesProps> = ({ company, services }) => {
  ...
  return (
    <section id="dich-vu" className="py-8 sm:py-24 bg-white dark:bg-slate-900 relative transition-colors duration-300">
        ...
        <div className="space-y-4 sm:space-y-8">
          {services.map((service, index) => {
            ...
            return (
              <div
                key={service.id}
                className={`grid grid-cols-1 lg:grid-cols-12 gap-3.5 lg:gap-8 items-center bg-slate-50 dark:bg-slate-800 rounded-xl sm:rounded-2xl p-3.5 sm:p-6 border border-slate-200/90 dark:border-slate-800 shadow-xs hover:shadow-md transition-all duration-300`}
              >
                {/* Image Col (4 cols) */}
                <div className={`lg:col-span-4 ${isEven ? "lg:order-2" : "lg:order-1"}`}>
                  <div className="relative rounded-lg sm:rounded-xl overflow-hidden shadow-sm aspect-[16/9] sm:aspect-[16/10] bg-slate-200 dark:bg-slate-950">
                    <ImageCarousel
                    ...
                    <div className="absolute bottom-2.5 left-2.5 sm:bottom-3 sm:left-3 bg-white/95 dark:bg-slate-900/95 backdrop-blur-md px-2 py-0.5 sm:px-2.5 sm:py-1 rounded-lg border border-slate-200 dark:border-slate-800 shadow text-[9px] sm:text-[10px] font-bold text-slate-800 dark:text-slate-200 flex items-center gap-1 sm:gap-1.5">
                      <ShieldAlert className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-emerald-600" />
                ...
                <div className={`lg:col-span-8 ${isEven ? "lg:order-1" : "lg:order-2"} text-left`}>
                  <div className="flex items-center gap-2.5 sm:gap-3 mb-2 sm:mb-2.5">
                    <div className="p-1.5 sm:p-2 bg-white dark:bg-slate-900 rounded-lg sm:rounded-xl shadow-xs border border-slate-200 dark:border-slate-700 shrink-0">
                      {getServiceIcon(service.icon)}
                    </div>
                    <div>
                      <span className="text-[9px] sm:text-[10px] font-extrabold text-[#F47A20] uppercase tracking-wider block">
                        Giải Pháp #{index + 1}
                      </span>
                  ...
                  <p className="text-xs sm:text-base text-slate-600 dark:text-slate-300 leading-relaxed mb-3 sm:mb-4">
                    {service.description}
                  </p>

                  {/* Bullet Points */}
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-1.5 sm:gap-y-2 mb-3.5 sm:mb-5 pl-0.5">
                  ...
                  <div className="flex items-center gap-2 sm:gap-2.5">
                    <a
                      ...
                      className="flex-1 sm:flex-none inline-flex items-center justify-center gap-1.5 bg-[#075FA8] hover:bg-[#0B1F33] dark:hover:bg-blue-600 text-white font-bold px-3 py-2 sm:px-3.5 sm:py-2.5 rounded-lg sm:rounded-xl shadow-xs transition-colors text-xs sm:text-sm whitespace-nowrap"
                    >
                    ...
                    <a
                      ...
                      className="flex-1 sm:flex-none inline-flex items-center justify-center gap-1.5 bg-white dark:bg-slate-800 hover:bg-slate-50 dark:hover:bg-slate-700 border border-slate-300 dark:border-slate-700 text-slate-700 dark:text-slate-200 font-bold px-3 py-2 sm:px-3.5 sm:py-2.5 rounded-lg sm:rounded-xl shadow-2xs transition-colors text-xs sm:text-sm whitespace-nowrap"
                    >
