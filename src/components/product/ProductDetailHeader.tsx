...
export const ProductDetailHeader: React.FC<ProductDetailHeaderProps> = ({
  ...
  return (
    <div className="flex items-center gap-1.5 sm:gap-2 text-xs sm:text-sm font-medium whitespace-nowrap overflow-x-auto no-scrollbar py-0.5 my-0 leading-none max-w-full">
      {/* Back button */}
      <button
        type="button"
        onClick={() => router.back()}
      ...
      {/* Breadcrumb line on Desktop, Direct page title on Mobile */}
      <nav aria-label="Breadcrumb" className="flex items-center gap-1.5 leading-none shrink-0 min-w-0">
        ...
        {categoryName && (
          <>
            ...
            <Link
              href={catHref}
              className="hidden sm:inline-flex items-center text-slate-600 dark:text-slate-400 hover:text-[#075FA8] dark:hover:text-blue-400 font-medium transition-colors shrink-0"
            >
        ...
        <span className="inline-flex items-center font-extrabold text-slate-900 dark:text-white truncate max-w-[200px] sm:max-w-md">
          {productName}
        </span>
      </nav>
    </div>
